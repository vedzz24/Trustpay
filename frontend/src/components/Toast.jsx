  ...
  return (
    <div className="fixed bottom-4 right-4 z-[200] flex flex-col gap-2 pointer-events-none max-w-sm w-full">
      <AnimatePresence>
        {toasts.map(t => {
          const cfg = {
            success: { Icon: CheckCircle,  cls: 'bg-cream-100 dark:bg-brown-500 text-success         border-green-200  dark:border-green-800' },
            error:   { Icon: AlertTriangle, cls: 'bg-cream-100 dark:bg-brown-500 text-danger          border-red-200    dark:border-red-800' },
            info:    { Icon: Info,          cls: 'bg-cream-100 dark:bg-brown-500 text-brown-400 dark:text-warning border-brown-200 dark:border-brown-400' },
          }[t.type || 'info'];
          return (
            <motion.div key={t.id} layout initial={{ opacity:0, y:16, scale:.95 }} animate={{ opacity:1, y:0, scale:1 }} exit={{ opacity:0, scale:.9 }}
              ...
              <p className="text-sm font-semibold flex-1 text-brown-500 dark:text-cream-200">{t.message}</p>
